...
class AudioPlayer: NSObject {
    ...
    /// situations:
    /// 1. the player is nil:
    ///     1. the player is nil by default, which means the playItems has not started playing yet.
    ///     2. the player is set to nil by the stop() method.
    ///         1. the playItems is changed
    ///         2. the playItems reaches the end and loopingStatus is .none
    /// 3. the player is not nil, which means the playItems has been started
    ...
    /// this should only be called when tap in the playItems menu, not the AudioListView
    // TODO: play audio at specified index of the playItems
    ...
    func playNext() {
        /// if it is the last audio in the playItems and looping status is none
        /// call the playNext will do nothing
        /// but leave it now and add a todo
        // TODO: handle the case when the audio list reaches the end
        /*
        if currentIndex + 1 == playItems.count,
           loopingStatus == .none {
            return
        }
         */

        currentIndex = (currentIndex + 1) % playItems.count
        invalidateCurrentPlayer()
        play()
    }
...
extension AudioPlayer {
    var currentAudio: AudioItem? {
        guard currentIndex >= 0 && currentIndex < playItems.count else {
            return nil
        }
        return playItems[currentIndex]
    }
}
